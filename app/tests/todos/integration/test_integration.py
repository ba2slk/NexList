...
def test_create_todo():
    response = requests.post(
        f"{BASE_URL}/todo", json={"task": "Test Task", "due_date": "2025-12-31"}
    )
...
def test_read_single_todo():
    requests.post(
        f"{BASE_URL}/todo", json={"task": "Single Task", "due_date": "2025-12-31"}
    )
...
def test_edit_todo():
    requests.post(
        f"{BASE_URL}/todo", json={"task": "Old Task", "due_date": "2025-12-31"}
    )
    response = requests.put(
        f"{BASE_URL}/todo/1", json={"task": "New Task", "due_date": "2026-01-01"}
    )
    assert response.status_code == 200
    data = response.json()
...
def test_toggle_todo_completed():
    requests.post(
        f"{BASE_URL}/todo", json={"task": "Toggle Task", "due_date": "2025-12-31"}
    )
...
def test_delete_single_todo():
    requests.post(
        f"{BASE_URL}/todo", json={"task": "Task to Delete", "due_date": "2025-12-31"}
    )
